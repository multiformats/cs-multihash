...
namespace Multiformats.Hash
{
    public class Multihash
    {
        ...
        public static Multihash Decode(byte[] buf)
        {
            ...
            {
                throw new Exception("Inconsistency length");
            }

            return new Multihash((HashType)code, buf.Slice(offset));
        }

        public static byte[] Encode(byte[] data, HashType code) => Binary.Varint.GetBytes((uint)code)
            .Concat(Binary.Varint.GetBytes((uint)data.Length), data);
        ...
        {
            var algorithm = Registry.GetHashType<TAlgorithm>();
            if (!algorithm.HasValue)
            {
                throw new NotSupportedException($"{typeof(TAlgorithm)} is not supported.");
            }

            return Binary.Varint.GetBytes((uint)algorithm.Value)
                .Concat(Binary.Varint.GetBytes((uint)data.Length), data);
        }

        private static Multihash Sum(IMultihashAlgorithm algo, byte[] data, int length) =>
            new(algo.Code, algo.ComputeHash(data, length).Slice(0, length != -1 ? length : algo.DefaultLength));

        public static Multihash Sum(HashType code, byte[] data, int length = -1) =>
            _registry.Use(code, algo => Sum(algo, data, length));
        ...
        private static Task<Multihash> SumAsync(IMultihashAlgorithm algo, byte[] data, int length) =>
            algo.ComputeHashAsync(data, length).ContinueWith(t =>
                new Multihash(algo.Code, t.Result.Slice(0, length != -1 ? length : algo.DefaultLength)));
        ...
        public static Task<Multihash> SumAsync<TAlgorithm>(byte[] data, int length = -1)
        ...
        public static string GetName(int code) =>
            Enum.IsDefined(typeof(HashType), (HashType)code)
                ? GetName((HashType)code)
                : "unsupported";

        public static HashType? GetCode(string name) =>
            Enum.TryParse(name.Replace("-", "_"), true, out HashType result)
                ? result
                : new HashType?();
    }
}
